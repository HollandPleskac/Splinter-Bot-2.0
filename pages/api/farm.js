...
async function getShouldBattle() {
  const doc = await firestore.collection('Users').doc('dpleskac@gmail.com').get()
...
async function farm() {
  ...
  while (await getShouldBattle() === true) {
    try {
      ...
      // BATTLE
      const splinterChoice = await firestore.collection('Users').doc('dpleskac@gmail.com').get().then(doc => doc.data().splinterChoice)
